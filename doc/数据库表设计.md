...
> 版本：v2.0 | 存储引擎：SQLite 3 | 模式：离线优先 + WebDAV 云同步
...
> `visual_type = 'image'` 时应有对应 `deal_images` 记录；`ascii` 时内容在 `deals.ascii_art`。
...
### 3.10 `ai_configs` — AI 对话配置

```sql
CREATE TABLE ai_configs (
  id                TEXT PRIMARY KEY,           -- 配置唯一标识
  provider_preset   TEXT NOT NULL DEFAULT 'custom',  -- 服务商预设 ID（deepseek / siliconflow / openai / claude / custom）
  protocol          TEXT NOT NULL DEFAULT 'openai',   -- 协议类型（openai / claude）
  api_key           TEXT NOT NULL DEFAULT '',          -- API Key（明文存储，v2 迁移至 secrets 表）
...
> 支持多套 AI 配置方案，`is_active = 1` 的为当前使用方案。API Key 在 v2 中迁移至 `secrets` 表存储，`api_key` 列保留兼容。
...
|----------|----------|-----------|-----------|------|
| `ai` | `api_key` | `sk-xxx` | `default` | AI 服务 API Key |
...
> **安全说明**：所有密钥密码以明文存储在本地 SQLite 数据库中，不进行加密。此设计基于离线优先原则，避免加密密钥管理复杂度。数据库文件受应用沙箱保护，不参与云同步。
...
**v1 → v2 迁移**：新增 `ai_configs` 和 `secrets` 表。
